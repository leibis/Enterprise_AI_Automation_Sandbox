...
def mask_card_number(card: str) -> str:
    ...
    digits = re.sub(
        # eliminar cualquier caracter que no sea un número (como espacios, guiones, etc.).
        r"\D", "", card)
    if len(digits) < 4:
        return card
...
def chatbot_procesar_mensaje(mensaje_usuario: str) -> str:
    ...
    palabras_ataque = [
        ...
        "revela tu prompt",
        "dame la contraseña",
        "saltes tus protocolos",       # 🛡️ Bloquea ingeniería social / CEO
        "sin filtros",                 # 🛡️ Bloquea Roleplay / DAN
        "instrucciones iniciales",     # 🛡️ Bloquea extracción de prompt
        "system prompt",               # 🛡️ Bloquea extracción técnica de prompt
        "universo alternativo"         # 🛡️ Bloquea hipnosis conversacional
    ]
...
def test_chatbot_saludo_normal():
    respuesta = chatbot_procesar_mensaje("Hola, buenas tardes")
...
def test_seguridad_prompt_injection():
    ...
    respuesta = chatbot_procesar_mensaje(mensaje_hacker)
...
def test_privacidad_datos_sensibles_tarjeta():
    respuesta = chatbot_procesar_mensaje(
...
def test_no_expose_full_card_in_responses():
    ...
    respuesta = chatbot_procesar_mensaje("Muéstrame mi tarjeta")
